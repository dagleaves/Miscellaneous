...
class demo(tk.Tk):
    def __init__(self):
        ...
        self.sheet = Sheet(self.frame,
                           ...
                           #header_align = "w",
                           #row_index_align = "w",
                           data=[[f"Row {r}, Column {c}\nnewline1\nnewline2" for c in range(
                               50)] for r in range(1000)],  # to set sheet data at startup
        ...
        self.menu = tk.Menu(self.sheet, tearoff=0)
        self.menu.add_command(label='Show details')
        self.menu.add_command(label='Add to collection')
        self.menu.add_command(label='Remove from collection')
        ...
        self.sheet.display_subset_of_columns(
            indexes=[0, 1, 2, 3, 4, 5], enable=True)
        #self.sheet.display_columns(enable = False)
        self.sheet.insert_column(idx=0)
        self.sheet.insert_columns(
            columns=5, idx=10, mod_column_positions=False)
        ...
        self.sheet.highlight_cells(
            row=5, bg="#ed4337", fg="white", canvas="row_index")
        self.sheet.highlight_cells(
            column=0, bg="#ed4337", fg="white", canvas="header")
        ...
        self.sheet.bind("<3>", self.do_popup)
    ...
    def begin_edit_cell(self, event):
        ...
        # return value is the text to be put into cell edit window
        return event[2]
